...
export const GET: APIRoute = async ({ params }) => {
  ...
  try {
    ...
    return jsonResponse({ error: error instanceof Error ? error.message : 'Failed to fetch record' }, 500);
  }
};
...
export const PATCH: APIRoute = async ({ params, request }) => {
  ...
  try {
    ...
    return jsonResponse(await updateRecord(collectionId, id, patch, resolveAuditActor(request)));
  } catch (error) {
